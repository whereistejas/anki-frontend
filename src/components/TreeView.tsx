  ...
  return (
    <section className="flex h-full min-h-0 max-h-[calc(100vh-1rem)] flex-1 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white md:max-h-[calc(100vh-2rem)]">
      <div className="flex items-center gap-2 border-b border-slate-100 px-3 py-2">
        <h2 className="text-base font-normal text-slate-900">{title}</h2>
      </div>

      {nodes.length === 0 ? (
        <div className="grid min-h-[8rem] place-items-center px-3 text-base font-normal text-slate-400">{emptyLabel}</div>
      ) : (
  ...
  return (
    <div>
      <div className="flex items-center gap-1" style={{ paddingLeft: `${level * 14}px` }}>
        ...
        <button
          className={`flex-1 truncate rounded px-2 py-1 text-left text-base font-normal transition ${
